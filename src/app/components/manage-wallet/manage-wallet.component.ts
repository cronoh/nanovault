...
export class ManageWalletComponent implements OnInit {
  ...
  async changePassword() {
    if (this.newPassword !== this.confirmPassword) return this.notifications.sendErrRemove(`Passwords do not match`);
    if (this.newPassword.length < 1) return this.notifications.sendErrRemove(`Password cannot be empty`);
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarninRemove(`Wallet must be unlocked`);
    ...
    this.notifications.sendSuccesRemove(`Wallet password successfully updated`);
  }
  ...
  async exportAddressBook() {
    ...
    if (exportData.length >= 25) {
      return this.notifications.sendErrRemove(`Address books with 25 or more entries need to use the file export method.`);
    }
  ...
  exportAddressBookToFile() {
    ...
    this.notifications.sendSuccesRemove(`Address book export downloaded!`);
  }
  ...
  exportToFile() {
    ...
    this.notifications.sendSuccesRemove(`Wallet export downloaded!`);
  }
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendErrRemove(`Bad import data, make sure you selected a MikronWebWallet Address Book export`)
        }
